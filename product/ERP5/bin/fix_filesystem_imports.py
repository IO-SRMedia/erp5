...
# -*- coding: utf-8 -*-
"""
This script fixes imports in a customer Git repository for {Document,
Interfaces,Mixin,Tool,Tests} migrated from filesystem to ZODB Components:
...
def ProductsToComponentModule(target_path, filesystem_module_list, regex_list):
  try:
  ...
  for filepath in filepath_list:
    if filepath:
      if filepath.endswith('.py'):
        ...
        print ("WARNING: Not fixing name in %s: Not a Python file (object not "
               "migrated to PortalTypeClass?)" % filepath)
...
def ERP5TypeInterfacesToInterfaceComponent(target_path, interface_name_list):
  ...
  for filepath in subprocess.check_output(r"git grep -lzE '%s' -- '*.py'" % '|'.join(interface_name_list),
                                          shell=True,
                                          cwd=target_path).split('\x00'):
    if not filepath:
      continue
...
def main():
  ...
  def __append(filesystem_module, component_module):
    ...
    regex_list.append(r"s#%s\>#%s#g" % (filesystem_module, component_module))
  for line in subprocess.check_output(SEARCH_COMMAND, shell=True, cwd=erp5_path).split('\n'):
    if line:
  ...
  if filesystem_module_list:
    ProductsToComponentModule(target_path, filesystem_module_list, regex_list)

  if interface_name_list:
    ERP5TypeInterfacesToInterfaceComponent(target_path, interface_name_list)
